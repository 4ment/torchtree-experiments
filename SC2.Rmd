...
## Probs
```{r gtr-probs}
ggplot(gtr_probs, aes(value, group = type, colour = type)) +
  my_theme +
  geom_density() +
...
```{r gtr-other}
g <- ggplot(gtr_other, aes(value, colour = type)) +
  my_theme +
  geom_density() +
  facet_wrap(~param, scales = "free") +
  labs(colour = NULL, x = "Value", y = "Density")
...
## Coalescent
```{r gtr-coalescent}
ggplot(gtr_thetas, aes(value, group = type, colour = type)) +
  my_theme +
  geom_density() +
  facet_wrap(~param, scales = "free") +
  labs(colour = NULL, x = "Value", y = "Density")
...
df <- df_beast_gtr %>%
  ...
  rename(
    logPopSize1 = skygrid.logPopSize1, logPopSize2 = skygrid.logPopSize2,
    logPopSize3 = skygrid.logPopSize3, logPopSize4 = skygrid.logPopSize4,
    logPopSize5 = skygrid.logPopSize5
  ) %>%
...
hky_other <- df_beast %>%
  ...
  rename(all_of(
    c(
      globalScale = "randomEffects.globalScale",
      rootHeight = "treeModel.rootHeight",
      precision = "skygrid.precision"
    )
  )) %>%
  mutate(type = "BEAST", model = "HKY-RE") %>%
  gather("param", "value", -type, -model)
...
## Probs
```{r hkre-probs}
ggplot(hky_probs, aes(value, group = type, colour = type)) +
  my_theme +
  geom_density() +
  facet_wrap(~param, scales = "free") +
  labs(colour = NULL, x = "Value", y = "Density")
...
## Coalescent
```{r hkyre-coalescent}
ggplot(hky_thetas, aes(value, group = type, colour = type)) +
  my_theme +
  geom_density() +
  facet_wrap(~param, scales = "free") +
  labs(colour = NULL, x = "Value", y = "Density")
...
## Other parameters
```{r hkyre-other}
g <- ggplot(hky_other, aes(value, group = type, colour = type)) +
  my_theme +
  geom_density() +
  facet_wrap(~param, scales = "free") +
  labs(colour = NULL, x = "Value", y = "Density")
...
df <- df %>%
  ...
  rename(all_of(lookup)) %>%
  select(all_of(params_id), contains("popSize"), starts_with("re"))
...
vars <- vars %>% mutate(time = beast_time / state_count * state, program = "BEAST")
cvs <- cvs %>% mutate(time = beast_time / state_count * state, program = "BEAST")
means <- means %>% mutate(time = beast_time / state_count * state, program = "BEAST")
```
...
df2 <- cvs %>%
  # filter(time < 2000) %>%
  select(starts_with("re"), all_of(c("state", "program", "time"))) %>%
  gather("param", "CV", -state, -time, -program) %>%
  filter(!is.nan(CV)) %>%
  mutate(param = factor(param, levels = c("reAC", "reAG", "reAT", "reCG", "reCT", "reGT", "reCA", "reGA", "reTA", "reGC", "reTC", "reTG")))
