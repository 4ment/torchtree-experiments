...
```{r}
parse_iterations <- function(filename) {
  v_log <- read_lines(filename)

  for (i in length(v_log):1) {
    a <- v_log[i] %>% str_split("\\s+")
    if (length(a[[1]]) == 5) {
...
```{r}
df_m <- fread(file.path("HCV-ELBO-100", "means.csv"),
  sep = ",",
  data.table = FALSE
)
df_v <- fread(file.path("HCV-ELBO-100", "variances.csv"),
  sep = ",",
  data.table = FALSE
)
...
```{r}
df2 <- cvs_extended %>% # filter(state > 100000) %>%
  select(all_of(c(params_id, "state", "program", "time"))) %>%
...
for (model in c("piecewise-linear", "skygrid")) {
  for (divergence in c("ELBO", "KLpq-10")) {
    for (program in c("physher")) {
      ...
      iterations <- parse_iterations(file.path(path, "torchtree.txt"))
      time <- parse_time(file.path(path, "torchtree.log"))
...
for (model in c("piecewise-linear", "skygrid")) {
  m <- if (model == "piecewise-linear") "skyglide" else model
  label <- paste("torchtree", m, "HMC", sep = "-")
...
```{r sky-plots}
skys2 = skys %>% mutate(proghow = str_replace(label, "-skyg\\w+", ""),
